...
def evaluate_boltzmann_policy(key, seed, config: MainConfig,  k=2):
    ...
    for timestep in range(num_timesteps):
        ...
        defender_action = get_boltzmann_action(subkey, particles, defender_actions, config, k)
...
def evaluate_H2S_policy(key, seed, config: MainConfig, H):

    np.random.seed(seed)  # Set seed for reproduceability

    # Initialize simulation components
    num_particles = config.filter.num_particles
    num_attacker_actions = int(config.game.num_attacker_actions)
...
def evaluate_AMG_policy(key, seed, config: MainConfig):
    ...
    # Generate trajectories for AMG (H2S = 0)
    trajectories = generate_all_trajectories(0, config)
...
def evaluate_MC_policy(key, seed, config: MainConfig):
    ...
    # Initialize simulation components
    num_particles = config.filter.num_particles
    num_attacker_actions = jnp.array(config.game.num_attacker_actions, int)
    num_defender_actions = config.game.num_defender_actions
    num_timesteps = config.game.num_timesteps
    ...
    for timestep in range(num_timesteps):
        ...
        key, subkey = jax.random.split(key)
        start_time = time.perf_counter()
        defender_action = get_MC_action_jax(model.probabilities, config, timestep, prev_atk_act, prev_def_act)
        end_time = time.perf_counter()
        ...
        #     key, subkey = jax.random.split(key)
        #     start_time = time.perf_counter()
        #     defender_action = get_MC_action_jax(model.probabilities, config, timestep)
        #     end_time = time.perf_counter()
        #     prev_atk_act = -1
        #     prev_def_act = -1
...
def evaluate_FP_policy(key, seed, config: MainConfig):
    ...
    for timestep in range(num_timesteps):
        ...
        def_model.update_model(attacker_action, timestep)
...
def evaluate_DM_policy(key, seed, config: MainConfig, tao=1):
    ...
    model = DroneAttacker(config)
    def_model = DirichletMultinomial(tao, config)
    ...
    for timestep in range(num_timesteps):
        ...
        key, subkey = jax.random.split(key)
        start_time = time.perf_counter()
        defender_action = def_model.select_action(prev_atk_act, prev_def_act)
        end_time = time.perf_counter()
