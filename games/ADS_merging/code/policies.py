...
def simulate_particle(key, particle, defender_action, num_attacker_actions, config):
    ...
    particles = _update_action_probabilities_masked(particle, K)
...
def simulate_particles(carry, defender_action):
    ...
    Note: The simulated particles are returned as they are utilized when looking into the next timestep
    """
    ...
    sim_particles, utilities = jax.vmap(simulate_particle, in_axes=(0, 0, None, None, None))(
        keys, particles, defender_action, num_attacker_actions, config
    )
    cum_utility = utility + jnp.sum(utilities)
    new_carry = (key, sim_particles, cum_utility, config)
