"Separate config file for generating ADP data"
...
class GameConfig:
    ...
    # defender_beta_1: float = 10.0
    # defender_beta_2: float = 2.0
    # defender_beta_3: float = 1.0
    # defender_beta_4: float = 2.0
    # defender_beta_5: float = 5.0
    # defender_beta_6: float = 0.0

    defender_beta_1: float = 20.0
    defender_beta_2: float = 2.0
    defender_beta_3: float = 1.0
    defender_beta_4: float = 2.0
    defender_beta_5: float = 5.0
    defender_beta_6: float = 0.01
    ...
    max_steering: float = 1.0
    max_heading: float = 3.0 #Ideally what we want our maximum heading in either direction to be
    road_margin: float = 0.25 #How close the agents can get to the road boundary before incurring a penalty
    wall_width: float = 1.0 #The steepness of the boundary penalty
    slot_gap: float = 4.0
    ...
    def create(key, **kwargs):
        ...
        IG_alpha: int = 2
        IG_beta: int = 1
        error_Z_sigma = IG_beta / random.gamma(key, IG_alpha)
...
class PriorConfig:
    ...
    XD1_parameters: tuple[float, float] = (2, 2)
    # XD1_bounds: tuple[int, int] = (-8, 16) #Horizontal Pos
    XD1_bounds: tuple[int, int] = (-30, 30) #Horizontal Pos
    ...
    XD3_bounds: tuple[int, int] = (-50, 50) #Heading

    XD4_parameters: tuple[float, float] = (4, 4)
    XD4_bounds: tuple[int, int] = (0, 50) #Speed

    XA1_parameters: tuple[float, float] = (3, 3)
    # XA1_bounds: tuple[int, int] = (-8, 16) #Horizontal Pos
    XA1_bounds: tuple[int, int] = (-50, 50) #Horizontal Pos
    ...
    XA3_parameters: tuple[float, float] = (5, 5)
    XA3_bounds: tuple[int, int] = (-30, 30) #Heading

    XA4_parameters: tuple[float, float] = (3, 3)
    XA4_bounds: tuple[int, int] = (0, 50) #Speed
...
def print_main_config(cfg, indent: int = 2):
    ...
    def print_section(name, section_dict):
        ...
        for k, v in section_dict.items():
            print(" " * indent + f"{k:<2}: {fmt_value(v)}")
